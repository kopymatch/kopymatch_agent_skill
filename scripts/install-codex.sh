#!/bin/bash
# install-codex.sh — Cài skill vào project cho Codex (.agents/skills)
# Usage: bash scripts/install-codex.sh /path/to/project
...
if [ -z "$PROJECT_DIR" ]; then
  ...
  echo "Usage: bash scripts/install-codex.sh /path/to/project"
  exit 1
fi
...
echo ""
echo "Codex sẽ tự phát hiện skills trong .agents/skills/"
